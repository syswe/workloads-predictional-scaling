...
def xgb_predict_next(values, lags):
    ...
    dtest = xgb.DMatrix(x_next)
    return float(bst.predict(dtest)[0])
...
def main():
    ...
    if not isinstance(replica_history, list) or len(replica_history) == 0:
        print("No evaluations provided to XGBoost algorithm", file=sys.stderr)
        sys.exit(1)
    ...
    for _ in range(steps_ahead):
        pred = xgb_predict_next(forecast_values, lags)
